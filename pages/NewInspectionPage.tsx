...
const NewInspectionPage: React.FC = () => {
    ...
    const [limitInfo, setLimitInfo] = useState<{ maxInspections: number; currentInspections: number; maxRooms: number; maxProperties?: number } | null>(null);
        ...
        try {
            ...
            if (user) {
                const { data: profile } = await supabase
                    ...
                    .select('*, plans:subscription_plan_id(max_inspections, max_rooms, max_properties)')
                    .eq('user_id', user.id)
                    .single();
        ...
        <div className="max-w-5xl mx-auto space-y-8 pb-20 animate-in fade-in duration-500">
            ...
            <div className="bg-white rounded-[40px] border border-slate-200 shadow-2xl shadow-slate-200/50 overflow-hidden">
                <div className="p-8 md:p-12">
                    ...
                    {step === 1 && (
                        <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                            ...
                            <div className="bg-slate-50 p-6 rounded-[32px] border border-slate-100 space-y-4">
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Selecione o Proprietário (Cliente)</label>
                                <div className="flex gap-4">
                                    <select value={lessorId} onChange={e => setLessorId(e.target.value)} className="flex-1 px-6 py-4 bg-white border border-slate-200 rounded-3xl outline-none focus:ring-4 focus:ring-blue-500/5 transition-all text-sm font-bold shadow-sm">
                                        <option value="">Selecione...</option>
                                        {clients.filter(c => c.profile_type === (reportType === 'Locação' ? 'Proprietário' : 'Vendedor') || !c.profile_type).map(c => (
                                            <option key={c.id} value={c.id}>{c.name}</option>
                                        ))}
                                    </select>
                                    <button onClick={() => setCreatingLessor(!creatingLessor)} className={`px-6 rounded-3xl border font-bold text-xs uppercase transition-all ${creatingLessor ? 'bg-slate-200 text-slate-600 border-slate-300' : 'bg-white text-blue-600 border-blue-200 hover:bg-blue-50'}`}>
                                        {creatingLessor ? 'Cancelar' : 'Novo'}
                                    </button>
                                </div>

                                {creatingLessor && (
                                    <div className="p-6 bg-white rounded-2xl shadow-lg border border-slate-100 space-y-4 animate-in zoom-in-95">
                                        <h4 className="font-bold text-slate-900 border-b pb-2">Cadastrar Novo Proprietário</h4>
                                        <div className="space-y-3">
                                            <input placeholder="Nome Completo" value={newClientName} onChange={e => setNewClientName(e.target.value)} className="w-full px-4 py-3 bg-slate-50 rounded-xl text-sm outline-none border border-slate-100 focus:border-blue-300" />
                                            <div className="grid grid-cols-2 gap-3">
                                                <input placeholder="Telefone" value={newClientPhone} onChange={e => setNewClientPhone(e.target.value)} className="w-full px-4 py-3 bg-slate-50 rounded-xl text-sm outline-none border border-slate-100 focus:border-blue-300" />
                                                <input placeholder="Email (Opcional)" value={newClientEmail} onChange={e => setNewClientEmail(e.target.value)} className="w-full px-4 py-3 bg-slate-50 rounded-xl text-sm outline-none border border-slate-100 focus:border-blue-300" />
                                            </div>
                                            <button onClick={() => handleCreateClient(reportType === 'Locação' ? 'Proprietário' : 'Vendedor')} className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold uppercase text-[10px] tracking-widest hover:bg-blue-700">Salvar Proprietário</button>
                                        </div>
                                    </div>
                                )}
                            </div>
                    ...
                    {step === 2 && (
                        <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                            ...
                            <div className="bg-slate-50 p-6 rounded-[32px] border border-slate-100 space-y-4">
                                ...
                                <div className="flex gap-4">
                                    <select value={propertyId} onChange={e => setPropertyId(e.target.value)} className="flex-1 px-6 py-4 bg-white border border-slate-200 rounded-3xl outline-none focus:ring-4 focus:ring-indigo-500/5 transition-all text-sm font-bold shadow-sm">
                                        <option value="">Selecione...</option>
                                        {/* Filter by owner if linking is implemented, for now list all or filter if possible */}
                                        {properties.map(p => (
                                            <option key={p.id} value={p.id}>{p.name} - {p.address}</option>
                                        ))}
                                    </select>
                                    <button onClick={() => setCreatingProperty(!creatingProperty)} className={`px-6 rounded-3xl border font-bold text-xs uppercase transition-all ${creatingProperty ? 'bg-slate-200 text-slate-600 border-slate-300' : 'bg-white text-indigo-600 border-indigo-200 hover:bg-indigo-50'}`}>
                                ...
                                {creatingProperty && (
                                    <div className="p-6 bg-white rounded-2xl shadow-lg border border-slate-100 space-y-4 animate-in zoom-in-95">
                                        ...
                                        <div className="space-y-3">
                                            <input placeholder="Apelido do Imóvel (ex: Ap do Centro)" value={newPropertyName} onChange={e => setNewPropertyName(e.target.value)} className="w-full px-4 py-3 bg-slate-50 rounded-xl text-sm outline-none border border-slate-100 focus:border-indigo-300" />
                                            <input placeholder="Endereço Completo" value={newPropertyAddress} onChange={e => setNewPropertyAddress(e.target.value)} className="w-full px-4 py-3 bg-slate-50 rounded-xl text-sm outline-none border border-slate-100 focus:border-indigo-300" />
                                            <select value={newPropertyType} onChange={e => setNewPropertyType(e.target.value)} className="w-full px-4 py-3 bg-slate-50 rounded-xl text-sm outline-none border border-slate-100 focus:border-indigo-300">
                                                <option value="Apartamento">Apartamento</option>
                                                <option value="Casa">Casa</option>
                                                <option value="Comercial">Comercial</option>
                                            </select>
                                            <button onClick={handleCreateProperty} className="w-full py-3 bg-indigo-600 text-white rounded-xl font-bold uppercase text-[10px] tracking-widest hover:bg-indigo-700">Salvar Imóvel</button>
                                        </div>
                                    </div>
                                )}
                            </div>
                    ...
                    {/* PASSO 3 - CONFIGURAÇÃO (Merges old Step 1 & 2 parts) */}
                    {step === 3 && (
                        <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                            ...
                            <div className="grid md:grid-cols-2 gap-8">
                                <div className="space-y-2">
                                    ...
                                    <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-3xl outline-none focus:ring-4 focus:ring-slate-500/5 transition-all text-sm font-bold" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Tipo</label>
                                    <div className="flex gap-2">
                                        {['Locação', 'Venda'].map(t => (
                                            <button key={t} onClick={() => setReportType(t as any)} className={`flex-1 py-4 rounded-3xl text-xs font-black uppercase border transition-all ${reportType === t ? 'bg-slate-800 text-white border-slate-800' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                                                {t}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <div className="md:col-span-2 bg-slate-50 p-6 rounded-[32px] border border-slate-100 space-y-4">
                                    ...
                                    <div className="flex gap-4">
                                        <select value={lesseeId} onChange={e => setLesseeId(e.target.value)} className="flex-1 px-6 py-4 bg-white border border-slate-200 rounded-3xl outline-none focus:ring-4 focus:ring-indigo-500/5 transition-all text-sm font-bold shadow-sm">
                                            <option value="">Selecione...</option>
                                            {clients.filter(c => c.profile_type === (reportType === 'Locação' ? 'Locatário' : 'Comprador') || !c.profile_type).map(c => (
                                                <option key={c.id} value={c.id}>{c.name}</option>
                                            ))}
                                        </select>
                                        <button onClick={() => setCreatingLessee(!creatingLessee)} className={`px-6 rounded-3xl border font-bold text-xs uppercase transition-all ${creatingLessee ? 'bg-slate-200 text-slate-600 border-slate-300' : 'bg-white text-emerald-600 border-emerald-200 hover:bg-emerald-50'}`}>
                                    ...
                                    {creatingLessee && (
                                        <div className="p-6 bg-white rounded-2xl shadow-lg border border-slate-100 space-y-4 animate-in zoom-in-95">
                                            ...
                                            <div className="space-y-3">
                                                <input placeholder="Nome Completo" value={newClientName} onChange={e => setNewClientName(e.target.value)} className="w-full px-4 py-3 bg-slate-50 rounded-xl text-sm outline-none border border-slate-100 focus:border-emerald-300" />
                                                <div className="grid grid-cols-2 gap-3">
                                                    <input placeholder="Telefone" value={newClientPhone} onChange={e => setNewClientPhone(e.target.value)} className="w-full px-4 py-3 bg-slate-50 rounded-xl text-sm outline-none border border-slate-100 focus:border-emerald-300" />
                                                    <input placeholder="Email (Opcional)" value={newClientEmail} onChange={e => setNewClientEmail(e.target.value)} className="w-full px-4 py-3 bg-slate-50 rounded-xl text-sm outline-none border border-slate-100 focus:border-emerald-300" />
                                                </div>
